...
public class BalancedBracketsChecker {
	...
	public Boolean check() {
		...
		if((length %2!=0)) {
		System.out.printf("Bracket Expression doesn't have Even Number of Characters: %d", length);
		System.out.println();
		return false;
		}
		Stack<Character> stack=new Stack<Character>();
		Set<Character> openBracketsSet= BalancedBracketsUtils.getOpenBracketCharsSet();
		Set<Character> closeBracketsSet= BalancedBracketsUtils.getCloseBracketCharsSet();
	...
	for(int index=0; index < bracketExpression.length(); index ++) {
        ...
        if(openBracketsSet.contains(visitedChar)) {
            ...
            Character openBracketChar = BalancedBracketsUtils.getOpenBracketChar(closeBracketChar);				
